...
async def format_html_code(*, html_code: str,
                           template_url: str,
                           session: aiohttp.ClientSession,
                           is_detailed: str) -> DataTaskOfSubtopic:
    ...
    while True:
        if time > 5:
            return None
        try:
            response = await get_answer_task(template_url=template_url,
                                             html_code=task_html,
                                             session=session,
