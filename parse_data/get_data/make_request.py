...
async def request_to_url(*, url: str, data: Dict[str, str | int],
                         n_issue: int, is_detailed: str):
    time = 2
    ...
    while True:
        ...
        try:
            ...
            response = await session.post(url, data=data,
                                          headers=headers_for_get_data_tasks)
            ...
            if 'prob_maindiv' in html:
                formatted_html_codes = await get_tasks_from_html(html=html,
...
async def main(*, urls_with_data, n_issue: int, is_detailed: str):
    async_requests = [
        request_to_url(url=url[0], data=url[1], n_issue=n_issue,
                       is_detailed=is_detailed) for url
        in urls_with_data]
...
if __name__ == '__main__':
    tasks = [('https://math-ege.sdamgia.ru/test?theme=79',
              {'ajax': '1', 'skip': 5}), (
