...
# Технологии бота
Этот бот написан на библиотеке aiogram. В качестве базы данных используется PostgreSQL.
# Запуск бота
1. Скачайте все необходимые зависимости:
```python
pip install -r requirements.txt
```
2. Создайте файл `.env` и скопируйте в него переменные из файла `.env.example`. После этого заполните значения переменных:
